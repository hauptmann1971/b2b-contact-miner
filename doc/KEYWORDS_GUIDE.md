...
> **Важно (актуальный код):**
> - При добавлении через UI или `KeywordService.add_keyword()` **автоперевод не выполняется** — в БД попадает одна строка с выбранными `language` / `country`.
> - Метод `generate_translations()` есть, но вызывается **только вручную** из кода (см. ниже).
> - В таблице `keywords` уникален **текст** `keyword` (`UNIQUE`), а не пара `(keyword, language)`.

## Обзор процесса

Каждая строка в `keywords` — один поисковый запрос для одного рынка (`language` + `country`). Пайплайн берёт pending-строки и ставит задачи `search_keyword` в очередь ([TASK_QUEUE.md](TASK_QUEUE.md)).
...
### Перевод и варианты (только вручную):

После `add_keyword()` **ничего не переводится**. Чтобы получить варианты на языках из `settings.LANGUAGES`, вызовите явно:

```python
translations = keyword_service.generate_translations("финтех стартап", source_lang="ru")
# затем сохраните нужные строки в keywords отдельными add_keyword / INSERT
```

Автоматического сохранения переводов в БД **нет**.

---

## 2️⃣ Генерация вариантов (API, не пайплайн)

`generate_translations()` создает **синонимы и связанные термины** (Google Translate + небольшой словарь):
