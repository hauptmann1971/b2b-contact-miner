...
**Минимум для пайплайна:** MySQL + `.env` + `python main.py` (очередь задач в таблице `task_queue`).

При полном запуске через `deploy/start_all*` (dev):

1. **MySQL** (Docker или удалённый хост) — обязательно
2. **Flask Web Server** (порт 5000) — UI
3. **Main Pipeline** — `python main.py` (вручную или `scripts/scheduler.py`)
4. **FastAPI Monitoring** (порт 8000) — опционально (`monitoring/healthcheck.py`)
5. **Task Scheduler** — опционально, cron-like запуск `run_pipeline()`
6. **Redis** (Docker) — **не** используется очередью приложения; только если вы сами подняли контейнер для других целей
